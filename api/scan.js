...
Key value factors: exact set name and year, rookie cards (RC), card condition, parallel versions, autograph or patch/relic cards, print run number on card, Pokemon first edition stamp bottom left, shadowless vs shadow border, error cards, graded cards in PSA/BGS/CGC slabs.`
  ...
  async function getEbayPrices(item, catId, token) {
    ...
    try {
      ...
      console.error('eBay Finding API error:', e.message);
    }
...
If nothing identifiable: {"total_items_spotted":0,"items":[]}`,
    ...
    if (!claudeRes.ok) {
      ...
      // Try to parse Anthropic's error JSON for specific error types
      try {
        const errJson = JSON.parse(errText);
        errType = errJson.error?.type || 'unknown';
        errMessage = errJson.error?.message || errMessage;
      } catch (_) {}

      // Map known Anthropic error types to user-readable messages
      const errorMap = {
        'overloaded_error': 'Claude is overloaded — try again in a moment',
        'rate_limit_error': 'Rate limit hit on Claude API — try again shortly',
        'invalid_request_error': 'Image rejected by Claude (likely too large or wrong format)',
        'authentication_error': 'Claude API key is invalid or expired',
        'permission_error': 'Claude API key lacks permission for this request',
        'not_found_error': 'Claude API endpoint not found',
        'api_error': 'Claude API internal error',
        'billing_error': 'Anthropic credit balance issue — check console.anthropic.com',
      };

      const userMessage = errorMap[errType] || `Claude error (${errType})`;

      console.error('Claude API error:', errType, errMessage);

      return res.status(claudeRes.status).json({
        error: userMessage,
        error_type: errType,
        error_detail: errMessage,
        http_status: claudeRes.status,
        source: 'anthropic',
      });
    }
    ...
    if (first === -1) return res.status(500).json({ error: 'No JSON returned from Claude.', error_type: 'parse_error', source: 'scan_handler' });
    ...
    try { ebayToken = await getEbayToken(); } catch(e) {
      console.error('eBay OAuth token fetch failed:', e.message);
    }
    ...
    console.error('scan.js fatal error:', err);
