...
Key value factors: exact set name and year, rookie cards (RC), card condition, parallel versions, autograph or patch/relic cards, print run number on card, Pokemon first edition stamp bottom left, shadowless vs shadow border, error cards, graded cards in PSA/BGS/CGC slabs.`
  ...
  async function browseActiveLookup(query, token) {
    ...
    try {
      ...
      if (prices.length === 0) return null;
      // Proper average of however many results returned (up to 10)
      const rawAvg = prices.reduce((a, b) => a + b, 0) / prices.length;
      // Apply 40% reduction to the average for realistic value
      const adjustedAvg = Math.round(rawAvg * 0.60);
      return {
        min: Math.round(Math.min(...prices) * 0.60),
        max: Math.round(Math.max(...prices) * 0.60),
        avg: adjustedAvg,
        count: prices.length,
        source: 'ebay_active',
        adjusted: true
