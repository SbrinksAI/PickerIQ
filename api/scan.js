...
Key value factors: Mint on Card (MOC) or Mint in Box (MIB), first run production markings, exact product line (vintage vs modern), country of manufacture stamp (Hong Kong = older), factory errors or color variants, all original accessories present, major lines: Star Wars, GI Joe, He-Man, Transformers, TMNT, Marvel, DC.`,
    vinyl: `You are an expert vinyl record reseller with 30 years of estate sale experience.
Identify every vinyl record visible and assess collector value.
Key value factors:
- Original pressings vs reissues — check label, matrix numbers in dead wax
- Key labels: Blue Note, Prestige, Sun, Chess, Factory, Stax, Atlantic, Motown originals command premiums
- Genre matters enormously: jazz, blues, punk, hip hop, soul original pressings most valuable
- Colored vinyl, picture discs, shaped records
- Promo copies marked Promotional Use Only or Not For Sale
- Audiophile editions: Mobile Fidelity (MFSL), Classic Records, Analogue Productions, DCC
- Import pressings — UK, Japanese, German often superior to US
- Condition of sleeve AND vinyl — both matter equally
- First pressings identified by label design, catalog number format, country of manufacture
- Sealed copies command significant premium`,

    comics: `You are an expert comic book reseller with 30 years of estate sale experience.
Identify every comic book visible and assess collector value.
Key value factors:
- KEY ISSUES: first appearances of characters, origin issues, death issues, first solo titles, first meetings of major characters, major storyline firsts — FLAG THESE CLEARLY with is_key_issue: true
- Distinguish cameo appearance vs full first appearance — full appearance worth significantly more
- Graded copies in CGC or CBCS slabs — grade number critical to value
- Condition: spine stress, staple rust, writing, tape, water damage all reduce value significantly
- Silver Age (1956-1969) most valuable, Bronze Age (1970-1984) next, Modern Age generally low value
- Publisher matters: Marvel and DC key issues most sought after
- Newsstand editions vs direct editions on key issues — newsstand commands premium
- Variant covers, retailer exclusives
- Signed copies with authentication (witnessed signatures worth more)
- Key Marvel: Amazing Fantasy #15, FF#1, X-Men #1, Hulk #1, Spider-Man first appearances
- Key DC: Action Comics #1 era, Batman #1, Detective Comics key issues, New Gods`,
...
Key value factors: exact set name and year, rookie cards (RC), card condition, parallel versions, autograph or patch/relic cards, print run number on card, Pokemon first edition stamp bottom left, shadowless vs shadow border, error cards, graded cards in PSA/BGS/CGC slabs.`
  ...
  async function browseActiveLookup(query, token) {
    ...
    try {
      ...
      if (prices.length === 0) return null;
      // Average of however many results returned (up to 15)
  ...
  // Strip bad descriptors from eBay search query
  function cleanQuery(query) {
    const stopWords = [
      'tape', 'original', 'edition', 'pressing', 'sealed', 'first', 'vintage',
      'rare', 'collectible', 'reissue', 'release', 'album', 'record', 'vinyl',
      'cd', 'cassette', 'vhs', 'blu-ray', 'bluray', 'dvd', 'book', 'hardcover',
      'paperback', 'signed', 'autographed', 'mint', 'near', 'excellent', 'good',
      'condition', 'copy', 'print', 'printing', 'import', 'japanese', 'promo',
      'promotional', 'limited', 'deluxe', 'special', 'anniversary', 'remastered',
      'expanded', 'complete', 'box', 'set', 'collection', 'volume', 'vol'
    ];
    const words = query.split(/\s+/);
    const filtered = words.filter(w => !stopWords.includes(w.toLowerCase().replace(/[^a-z0-9]/g, '')));
    // Keep max 5 words
    return filtered.slice(0, 5).join(' ').trim();
  }

  // eBay lookup — try all sources, apply 40% reduction to whatever comes back
  async function getEbayPrices(item, catId, token) {
    ...
    const author = item.artist_author || '';
    const rawQuery = item.ebay_search_query || `${title} ${author}`.trim();
    const fullQuery = cleanQuery(rawQuery);
    const titleAuthorQuery = cleanQuery(`${title} ${author}`.trim());
    const titleOnlyQuery = cleanQuery(title.trim());

    // Attempt 1: full specific query → sold
    let prices = await findingSoldLookup(fullQuery, catId);

    // Attempt 2: title + author → sold
    if (!prices && titleAuthorQuery !== fullQuery) {
      prices = await findingSoldLookup(titleAuthorQuery, catId);
    }

    // Attempt 3: title only → sold
    if (!prices && titleOnlyQuery !== titleAuthorQuery) {
      prices = await findingSoldLookup(titleOnlyQuery, catId);
    }

    // Attempt 4: Browse API active listings fallback
    if (!prices) {
      prices = await browseActiveLookup(titleOnlyQuery || fullQuery, token);
    }

    // No reduction applied — use raw average from eBay's default relevance ranking

    cache[cacheKey] = { prices, ts: Date.now() };
    return prices;
  }

  // Tier from adjusted average
  // high   = High Potential Value   ($50+)
  // medium = Medium Potential Value ($10–$49)
  // low    = Low Potential Value    (under $10)
  ...
  "items": [
    {
      ...
      "ebay_search_query": "SHORT eBay search — title and artist/author ONLY, max 4 words, NO descriptors like tape, original, edition, pressing, sealed, first, vintage, rare, collectible, reissue, release",
...
- high = High Potential Value: strong collector demand, known valuable title, first edition, sealed, rare pressing, significant artist
- medium = Medium Potential Value: some collector interest, common valuable title, good condition indicators
- low = Low Potential Value: marginal interest, common item, worth checking but unlikely to be significant

Only include items with any potential resale value.
...
If nothing identifiable: {"total_items_spotted":0,"items":[]}`,
    ...
    try {
      ...
      // Try to salvage partial JSON by truncating at last complete item
      const partial = raw.slice(first);
      const lastComma = partial.lastIndexOf('},');
      if (lastComma > 0) {
        try {
          parsed = JSON.parse(partial.slice(0, lastComma + 1) + ']}');
        } catch(e2) {
          return res.status(500).json({ error: 'JSON parse error — try again or use a simpler photo' });
        }
      } else {
        return res.status(500).json({ error: 'JSON parse error — try again or use a simpler photo' });
      }
    ...
    // Sort by tier: high first, then medium, then low
    ...
    itemsWithUrls.sort((a, b) => {
      return (tierOrder[a.value_tier] ?? 2) - (tierOrder[b.value_tier] ?? 2);
    });
