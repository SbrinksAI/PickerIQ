...
Key value factors: exact set name and year, rookie cards (RC), card condition, parallel versions, autograph or patch/relic cards, print run number on card, Pokemon first edition stamp bottom left, shadowless vs shadow border, error cards, graded cards in PSA/BGS/CGC slabs.`
  ...
  // V7 eBay lookup: Browse API active listings with 40% reduction for realistic value
  async function getEbayPrices(item, catId, token) {
    ...
    // Primary: Browse API active listings
    let prices = await browseActiveLookup(query, token);

    // Fallback: Finding API if Browse fails
    if (!prices) {
      prices = await findingSoldLookup(query, catId);
    }

    // Apply 40% reduction to active listing prices for realistic value estimate
    if (prices && prices.source === 'ebay_active') {
      prices = {
        ...prices,
  ...
  // Assign value tier from adjusted eBay average
  // high = Possible High Value ($50+)
  // medium = Possible Good Value ($10-$49)
  // low = Low Value (under $10)
